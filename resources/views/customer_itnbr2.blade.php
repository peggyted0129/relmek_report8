...
<div class="container-lg mt-7">
  <form method="POST" action="{{ route('customer.itnbr2') }}">
    @csrf
    <div class="form-row">
      ...
      <div class="form-group col-6 col-sm-3 col-md-2">
        <label for="itnbr">品號 : </label>
        @if( empty($inputItnbr) )
          <input type="text" class="form-control" id="itnbr" name="itnbr" required>
        @else
          <input type="text" class="form-control" id="itnbr" name="itnbr" required value={{ $inputItnbr }}>
        @endif
      </div>
      <div class="form-group col-6 col-sm-3 col-md-2">
        <label for="itnbr2">品號 : </label>
        @if( empty($inputItnbr2) )
          <input type="text" class="form-control" id="itnbr2" name="itnbr2" required>
        @else
          <input type="text" class="form-control" id="itnbr2" name="itnbr2" required value={{ $inputItnbr2 }}>
        @endif
      </div>
      <div class="form-group col-10 col-sm-6 col-md-4">
...
<div class="container-fluid mt-6 mb-19" style="font-size:14px">
  @if( !empty($customer) )
    <div class="mb-3">查詢區間 : {{ $shpdate['inputStart'] }}~{{ $shpdate['inputEnd'] }} ， 品號 : {{ $inputItnbr }}、{{ $inputItnbr2 }} </div>
  @endif
